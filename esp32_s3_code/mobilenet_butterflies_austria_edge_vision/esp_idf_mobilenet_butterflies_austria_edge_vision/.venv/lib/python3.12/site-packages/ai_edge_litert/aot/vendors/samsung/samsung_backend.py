...
"""Samsung backend implementation for LiteRT AOT compiler."""
...
from ai_edge_litert.aot.core import aot_types as types
...
from ai_edge_litert.aot.vendors.samsung import target as target_lib

COMPILER_PLUGIN_LIB_PATH = pathlib.Path(
    "vendors/samsung/compiler/libLiteRtCompilerPlugin_Samsung.so"
)


@import_vendor.register_backend
class SamsungBackend(types.Backend):
  ...
  def soc_manufacturer(self) -> target_lib.SocManufacturer:
    return target_lib.SocManufacturer.SAMSUNG
  ...
  def specialize(self) -> Iterable["SamsungBackend"]:
  ...
  @classmethod
  def id(cls) -> str:
    return target_lib._SAMSUNG_BACKEND_ID  # pylint: disable=protected-access

  @classmethod
  def create(cls, config: types.Config) -> "SamsungBackend":
