...
uint32_t OpTraceFnv1a(const void* data, size_t size) {
...
// only defined for builds with the error strings.
    ...
    if (invoke_status == kTfLiteOk && node->outputs != nullptr) {
      for (int output_idx = 0; output_idx < node->outputs->size; ++output_idx) {
        const int tensor_idx = node->outputs->data[output_idx];
        if (tensor_idx < 0) {
          continue;
        }
        TfLiteEvalTensor* output =
            &subgraph_allocations_[subgraph_idx].tensors[tensor_idx];
        size_t output_bytes = 0;
        if (output->data.raw == nullptr ||
            TfLiteEvalTensorByteLength(output, &output_bytes) != kTfLiteOk) {
          continue;
        }
        int min_value = 0;
        int max_value = 0;
        int32_t sum_value = 0;
        if (output_bytes > 0 && output->type == kTfLiteInt8) {
          const int8_t* values = output->data.int8;
          min_value = values[0];
          max_value = values[0];
          for (size_t value_idx = 0; value_idx < output_bytes; ++value_idx) {
            const int value = values[value_idx];
            min_value = value < min_value ? value : min_value;
            max_value = value > max_value ? value : max_value;
            sum_value += value;
          }
        } else if (output_bytes > 0 && output->type == kTfLiteUInt8) {
          const uint8_t* values = output->data.uint8;
          min_value = values[0];
          max_value = values[0];
          for (size_t value_idx = 0; value_idx < output_bytes; ++value_idx) {
            const int value = values[value_idx];
        ...
        MicroPrintf(
            "OPTRACE op=%d name=%s tensor=%d bytes=%d hash=%x min=%d max=%d sum=%d",
            static_cast<int>(i), OpNameFromRegistration(registration),
            tensor_idx, static_cast<int>(output_bytes),
            static_cast<unsigned>(OpTraceFnv1a(output->data.raw, output_bytes)),
            min_value, max_value, static_cast<int>(sum_value));
      }
    }
