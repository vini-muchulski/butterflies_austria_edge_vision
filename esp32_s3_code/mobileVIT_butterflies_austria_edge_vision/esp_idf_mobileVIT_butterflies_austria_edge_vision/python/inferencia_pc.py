...
MODEL_PATH = Path(os.environ.get(
    "MODEL_PATH",
    BASE_DIR.parent / "data" / "mobilevit_int8.tflite",
))
...
def create_interpreter(model_path=MODEL_PATH, preserve_all_tensors=False):
    ...
    if list(input_details["shape"]) != [1, 3, 256, 256] or input_details["dtype"] != np.int8:
        raise ValueError("Entrada esperada: int8 [1,3,256,256]")
